...
function renderMenu($currentPage = '') {
    ...
    foreach ($menuItems as $item) {
        ...
        if (isset($item['page_id']) && !in_array($item['page_id'], $_SESSION['menu_access'])) {
            continue; // ข้ามเมนูที่ไม่มีสิทธิ์
        }
        
        // แสดงเมนูที่มีสิทธิ์เท่านั้น
        echo '<a href="'.$item['href'].'" class="flex items-center px-4 py-2.5 text-white hover:bg-white/10 rounded-lg transition-all duration-200">
